...
public class RcwlPrLineImportValidator extends PrLineImportValidator {
    ...
    public boolean validate(String data) {
        ...
        if (prHeader == null && !prHeader.getTenantId().equals(tenantId)) {
            this.addErrorMsg("error.data_invalid", prLineImportVO, null);
            return false;
        ...
        if (!prLineImportVO.isValidFlag()) {
            ...
            if (StringUtils.isEmpty(prLineImportVO.getItemCode())) {
                if (StringUtils.isEmpty(prLineImportVO.getItemName())) {
                    this.addErrorMsg("sprm.pr_line_import.item_code_and_name_be_lost_error", prLineImportVO, null);
                    return false;
                }

                this.codeValid(prLineImportVO, this.prImportMapper::queryUomInfo, prLineImportVO.getUomCode(), PrLineImportVO.FIELD_NAME_MAP.get("uomCode"), "sprm.pr_line_import.uom_error", true);
            } else {
                this.codeValid(prLineImportVO, this.prImportMapper::queryItemInfo, prLineImportVO.getItemCode(), PrLineImportVO.FIELD_NAME_MAP.get("itemCode"), "sprm.pr_line_import.item_error", false);
            }

            if (!prLineImportVO.isValidFlag()) {
                return false;
            } else {
                this.codeValid(prLineImportVO, this.prImportMapper::queryCategoryInfo, prLineImportVO.getCategoryCode(), PrLineImportVO.FIELD_NAME_MAP.get("categoryCode"), "sprm.pr_line_import.category_error", false);
                if (!prLineImportVO.isValidFlag()) {
                    return false;
                } else {
                    if (prLineImportVO.getNeededDate() == null) {
                        this.addErrorMsg("sprm.pr_line_import.needed_date_error", prLineImportVO, null);
                    }
                    ...
                    if (!prLineImportVO.isValidFlag()) {
                        ...
                        if (!prLineImportVO.isValidFlag()) {
                            ...
                            this.codeValid(prLineImportVO, this.prImportMapper::queryTaxInfo, prLineImportVO.getTaxCode(), PrLineImportVO.FIELD_NAME_MAP.get("taxCode"), "sprm.pr_line_import.tax_error", false);
                            if (!prLineImportVO.isValidFlag()) {
                                return false;
                            } else {
                                if (StringUtils.isNotEmpty(prLineImportVO.getExpBearDepUnitCode())) {
                                    ...
                                    if (unitVO == null) {
                                        ...
                                        if (unitVO == null) {
                                            ...
                                            if (unitVO == null) {
                                                this.addErrorMsg("sprm.pr_line_import.unit_error", prLineImportVO, PrLineImportVO.FIELD_NAME_MAP.get("expBearDepUnitCode"));
                                            }
                                        }
                                    }
                                }
                                ...
                                if (!prLineImportVO.isValidFlag()) {
                                    ...
                                    if (!prLineImportVO.isValidFlag()) {
                                        ...
                                        this.codeValid(prLineImportVO, this.prImportMapper::queryCostInfo, prLineImportVO.getCostCode(), PrLineImportVO.FIELD_NAME_MAP.get("costCode"), "sprm.pr_line_import.cost_error", false);
                                        if (!prLineImportVO.isValidFlag()) {
                                            return false;
                                        } else {
                                            this.codeValid(prLineImportVO, this.prImportMapper::queryAccountSubjectInfo, prLineImportVO.getAccountSubjectNum(), PrLineImportVO.FIELD_NAME_MAP.get("accountSubjectNum"), "sprm.pr_line_import.account_subject_error", false);
                                            if (!prLineImportVO.isValidFlag()) {
                                                return false;
                                            } else {
                                                this.userValid(prLineImportVO, this.prImportMapper::queryUserInfo, prLineImportVO.getKeeperUserLoginName(), PrLineImportVO.FIELD_NAME_MAP.get("keeperUserLoginName"), "sprm.pr_line_import.keeper_user_error");
                                                if (!prLineImportVO.isValidFlag()) {
                                                    return false;
                                                } else {
                                                    this.userValid(prLineImportVO, this.prImportMapper::queryUserInfo, prLineImportVO.getAccepterUserLoginName(), PrLineImportVO.FIELD_NAME_MAP.get("accepterUserLoginName"), "sprm.pr_line_import.accepter_user_error");
                                                    if (!prLineImportVO.isValidFlag()) {
                                                        return false;
                                                    } else {
                                                        this.codeValid(prLineImportVO, this.prImportMapper::queryAccountAssignType, prLineImportVO.getAccountAssignTypeCode(), PrLineImportVO.FIELD_NAME_MAP.get("accountAssignTypeCode"), "sprm.pr_line_import.account_assign_error", false);
                                                        this.codeValid(prLineImportVO, this.prImportMapper::queryWbs, prLineImportVO.getWbs(), PrLineImportVO.FIELD_NAME_MAP.get("accountAssignTypeCode"), "sprm.pr_line_import.wbs", false);

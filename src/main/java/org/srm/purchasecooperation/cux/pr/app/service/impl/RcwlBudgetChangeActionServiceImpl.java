        ...
        if (!CollectionUtils.isEmpty(rcwlBudgetChangeActions)) {
            ...
            if (rcwlBudgetChangeActions.size() == rcwlBudgetDistributions.size()) {
                ...
                long notConsistentCount = rcwlBudgetChangeActions.stream().filter(rcwlBudgetChangeAction -> !rcwlBudgetDistributionYearMap.containsKey(rcwlBudgetChangeAction.getBudgetDisYear())
                        || rcwlBudgetChangeAction.getLineAmount().setScale(RcwlBudgetDistribution.SIX, RoundingMode.HALF_UP).compareTo(rcwlBudgetDistributions.stream().map(RcwlBudgetDistribution::getBudgetDisAmount).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(RcwlBudgetDistribution.SIX, RoundingMode.HALF_UP)) != 0
